...
describe('test calculate logic for different cases', () => {
  ...
  test('sum 2 + 2 = 4', () => {
    ...
    expect(resultObj).toStrictEqual({total: '4', next: null, operation: null});
  });
  ...
  test('when receiving AC all values null', () => {
    ...
    const key = 'AC';
    const resultObj = calculate(obj,key);
    expect(resultObj).toStrictEqual({total: null, next: null, operation: null});
  });
  ...
  test('when theres only primary value and then press an operation  button', () => {
    ...
    const key = '-';
    const resultObj = calculate(obj,key);
    expect(resultObj).toStrictEqual({total: '100', next: null, operation: '-'});
  });
